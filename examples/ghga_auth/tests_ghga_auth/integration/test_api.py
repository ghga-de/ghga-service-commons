...
def get_headers(admin: bool = False) -> dict[str, str]:
    """Get a request headers with an auth token for testing."""
    claims = {
        "name": "John Doe",
        "email": "john@home.org",
        "title": "Dr.",
...
def test_require_auth_authenticated_but_not_admin(client):
    """Test the require_auth endpoint authenticated, but not as admin."""
    response = client.get("/require_admin", headers=get_headers())
    assert response.status_code == status.HTTP_403_FORBIDDEN
    assert response.json() == {"detail": "Not authorized"}


def test_require_auth_authenticated_as_admin(client):
    """Test the require_auth endpoint authenticated as admin."""
